...
export const metadata = {
  title: "Self-hosting",
  description: "Deploy WallCab with Vercel, a Cloudflare Worker, and Workers KV.",
...
The MVP deployment uses Vercel for Next.js and a free Cloudflare Worker with KV for generated image caching. The website can run without the Worker, but each image request will render independently.
...
No database, paid API, billing credential, or content-provider key is required for the MVP. Active-day KV records hold only public lesson, attribution, and image data and expire at the next UTC rollover.
...
Promote a preview only after exercising the configurator, API redirect, source credits, and Shortcut flow.
